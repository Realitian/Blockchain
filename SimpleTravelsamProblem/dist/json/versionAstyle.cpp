...
public:
    ...
    // Operateur d'égalité
    bool operator==(Tour other) const
    {
    ...
    friend std::ostream& operator<< (std::ostream& os, const Tour& rhs)
    {
        ...
        for_each(cities.begin(),cities.end(), [&os](City b)
        {
            os << b;
        });
        return os;
    ...
    {
        if(fitness() == 0) return 1/distance();
        return fitness_;
    };

    // Set Fitness
    inline void setFitness(double fit)
    {
        fitness_ = fit;
    };

    // Set City at pos
    inline void setCity(int pos,const City& city)
    {
        if(pos >= size()) ajouterVille(pos);
        cities_.at(pos) = city;
    };
    ...
    // get City à pos
    inline City  getCity(int pos)    const
    {
        return cities_.at(pos);
    };
    ...
    */
    ...
    {
        auto res = std::find(cities_.begin(),cities_.end(),city);
        return (res != cities_.end());
    }
...
public:
    ...
    inline Tour getTour(int pos)    const
    {
        return tours_.at(pos);
    };
    // Obtenir le nombre de Tour
    inline void setTour(int pos,const Tour& tour)
    {
        if(pos >= tours_.size()) ajouterTour(pos);
        tours_.at(pos)=tour;
    };
    ...
    inline Tour getFittest()
    {
        std::partial_sort(tours_.begin(),tours_.begin()+2,tours_.end());
        return tours_.at(0);
    };
